...
|---|-------|------|--------|--------|----------|
| 3 | ProviderScope Wiring & ProductRepository Toggle | S | `backend` `infra` | ⏳ Ready | @devmahnx |

### In Progress

| # | Title | Size | Labels | Status | Assignee |
|---|-------|------|--------|--------|----------|
| 1 | Harden CI & Analyzer | S | `infra` `critical` `tests` | 🚧 In Progress | [GeoAziz/flutter-storefront/issues/9](https://github.com/GeoAziz/flutter-storefront/issues/9) (@GeoAziz) |
| 2 | Route Name Standardization | S | `infra` `critical` | 🚧 In Progress | [GeoAziz/flutter-storefront/issues/10](https://github.com/GeoAziz/flutter-storefront/issues/10) (@GeoAziz) |
...
| 6 | Checkout Skeleton & Payment Adapter | M | `backend` `enhancement` | — | #4 (Cart), #3 |
| 7 | Add End-to-End Smoke Tests | M | `tests` `critical` | — | #1 (CI) |
| 8 | Backend Contract: Products API | S | `backend` `critical` | — | #3 (ProviderScope) |
...
**Next Action:** Create GitHub Issues #1–8, assign to @devmahnx, begin work on #1 & #2 in parallel.
