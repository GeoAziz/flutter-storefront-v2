...
class ProfileMetrics {
  ...
  void calculateAverages() {
    ...
    avgMemoryMB = systemSamples.map((s) => s.memoryMB).reduce((a, b) => a + b) /
        systemSamples.length;
    avgCPU = systemSamples.map((s) => s.cpuPercent).reduce((a, b) => a + b) /
        systemSamples.length;
  }
...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    final systemTimer = Timer.periodic(Duration(seconds: 2), (timer) async {
      if (systemMonitoringStopwatch.elapsedMilliseconds >
          maxMonitoringDurationSeconds * 1000) {
        timer.cancel();
        return;
      }
      ...
      try {
        ...
        print(
            '[$memStatus] Memory: ${sample.memoryMB.toStringAsFixed(1)}MB | CPU: ${sample.cpuPercent.toStringAsFixed(1)}%');
        logSink.writeln(
            '[SYSTEM] Memory: ${sample.memoryMB.toStringAsFixed(1)}MB | CPU: ${sample.cpuPercent.toStringAsFixed(1)}% | ${sample.timestamp.toIso8601String()}');
      } catch (e) {
    ...
    if (metrics.peakMemoryMB < 50) {
      ...
      print(
          '❌ PROFILING FAILED: Memory usage exceeded target (${metrics.peakMemoryMB.toStringAsFixed(2)}MB > 50MB)');
    }
  } catch (e, st) {
    print('ERROR: $e');
    print(st);
...
Future<SystemSample> _captureSystemStats() async {
  try {
    ...
    final result =
        await Process.run('ps', ['aux'], runInShell: Platform.isWindows);
    ...
    final estimatedMemoryMB =
        (totalMemoryPercent / 100) * (systemMemoryGB * 1024);
